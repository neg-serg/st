...
static void (*handler[LASTEvent])(XEvent *) = {
        [KeyPress] = kpress,           [ClientMessage] = cmessage,
        [ConfigureNotify] = resize,    [VisibilityNotify] = visibility,
        [UnmapNotify] = unmap,         [Expose] = expose,
        [FocusIn] = focus,             [FocusOut] = focus,
        [MotionNotify] = bmotion,      [ButtonPress] = bpress,
        [ButtonRelease] = brelease,    [SelectionClear] = selclear,
...
size_t utf8len(char *c) { return utf8decode(c, &(long){0}, UTF_SIZ); }
...
void selsnap(int mode, int *x, int *y, int direction) {
    int newx, newy, xt, yt;
        ...
        case SNAP_WORD:
            ...
            for (;;) {
                ...
                if (!(gp->mode & ATTR_WDUMMY) &&
                    (delim != prevdelim || (delim && gp->c[0] != prevgp->c[0])))
                    break;
...
void selnotify(XEvent *e) {
    ...
    do {
        ...
        if (IS_SET(MODE_BRCKTPASTE)) ttywrite("\033[200~", 6);
        ttysend((char *)data, nitems * format / 8);
        if (IS_SET(MODE_BRCKTPASTE)) ttywrite("\033[201~", 6);
        XFree(data);
...
void clippaste(const Arg *dummy) {
    ...
    XConvertSelection(xw.dpy, clipboard, sel.xtarget, XA_PRIMARY, xw.win,
                      CurrentTime);
}
...
void selrequest(XEvent *e) {
    ...
    if (xsre->target == xa_targets) {
        ...
        XChangeProperty(xsre->display, xsre->requestor, xsre->property,
                        xsre->target, 8, PropModeReplace, (uchar *)sel.clip,
                        strlen(sel.clip));
        xev.property = xsre->property;
    }
...
void csihandle(void) {
    ...
    switch (csiescseq.mode) {
        ...
        case 'c': /* DA -- Device Attributes */
            if (csiescseq.arg[0] == 0) ttywrite(vtiden, sizeof(vtiden) - 1);
            break;
...
void tdumpline(int n) {
    ...
    if (bp != end || bp->c[0] != ' ') {
        for (; bp <= end; ++bp) tprinter(bp->c, utf8len(bp->c));
    }
    tprinter("\n", 1);
}
...
void tputc(char *c, int len) {
    ...
    if (sel.ob.x != -1 && BETWEEN(term.c.y, sel.ob.y, sel.oe.y)) selclear(NULL);
...
void xresize(int col, int row) {
    ...
    xw.buf =
        XCreatePixmap(xw.dpy, xw.win, xw.w, xw.h, DefaultDepth(xw.dpy, xw.scr));
...
int xsetcolorname(int x, const char *name) {
    ...
    if (!name) {
        if (BETWEEN(x, 16, 16 + 215)) { /* 256 color */
            ...
            if (!XftColorAllocValue(xw.dpy, xw.vis, xw.cmap, &color, &ncolor)) {
                return 1;
            }
    ...
    if (!XftColorAllocName(xw.dpy, xw.vis, xw.cmap, name, &ncolor)) return 1;
...
void xloadfonts(char *fontstr, double fontsize) {
    ...
    if (xloadfont(&dc.bfont, pattern)) die("st: can't open font %s\n", fontstr);
...
void xinit(void) {
    ...
    if (xw.gm & XNegative) xw.l += DisplayWidth(xw.dpy, xw.scr) - xw.w - 2;
    if (xw.gm & YNegative) xw.t += DisplayWidth(xw.dpy, xw.scr) - xw.h - 2;
    ...
    xw.win = XCreateWindow(
        xw.dpy, parent, xw.l, xw.t, xw.w, xw.h, 0,
...
void xdraws(char *s, Glyph base, int x, int y, int charlen, int bytelen) {
    ...
    if (base.mode & ATTR_ITALIC) {
        if (base.fg == defaultfg) base.fg = defaultitalic;
        font = &dc.ifont;
        frcflags = FRC_ITALIC;
        ...
        if (base.fg == defaultfg) base.fg = defaultunderline;
    }
    ...
    for (xp = winx; bytelen > 0;) {
        ...
        for (;;) {
            ...
            if (doesexist) {
                u8fl++;
                u8fblen += u8cblen;
                if (!oneatatime && bytelen > 0) continue;
            }

            if (u8fl > 0) {
                XftDrawStringUtf8(xw.draw, fg, font->match, xp,
                                  winy + font->ascent, (FcChar8 *)u8fs,
                                  u8fblen);
                xp += xw.cw * u8fl;
            }
            break;
        ...
        XftDrawStringUtf8(xw.draw, fg, frc[i].font, xp,
                          winy + frc[i].font->ascent, (FcChar8 *)u8c, u8cblen);
    ...
    */
    ...
    if (base.mode & ATTR_STRUCK) {
        XftDrawRect(xw.draw, fg, winx, winy + 2 * font->ascent / 3, width, 1);
    }
...
void xdrawcursor(void) {
    ...
    if (term.line[oldy][oldx].mode & ATTR_WDUMMY) oldx--;
    if (term.line[term.c.y][curx].mode & ATTR_WDUMMY) curx--;
    ...
    xdraws(term.line[oldy][oldx].c, term.line[oldy][oldx], oldx, oldy, width,
           sl);
...
void drawregion(int x1, int y1, int x2, int y2) {
    ...
    for (y = y1; y < y2; y++) {
        ...
        for (x = x1; x < x2; x++) {
            ...
            if (ena_sel && selected(x, y)) new.mode ^= ATTR_REVERSE;
            if (ib > 0 &&
                (ATTRCMP(base, new) || ib >= DRAW_BUF_SIZ - UTF_SIZ)) {
...
void focus(XEvent *ev) {
    ...
    if (ev->type == FocusIn) {
        ...
        xseturgency(0);
        if (IS_SET(MODE_FOCUS)) ttywrite("\033[I", 3);
    } else {
        XUnsetICFocus(xw.xic);
        xw.state &= ~WIN_FOCUSED;
        if (IS_SET(MODE_FOCUS)) ttywrite("\033[O", 3);
    }
}
